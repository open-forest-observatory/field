...
# load convex hulls
plots = st_read(file.path(datadir, "field-plot-locs/es/es-clusters.gpkg")) |>
  mutate(plot_id = paste0("ES-", str_pad(priority, side = "left", width = 2, pad = "0"))) |>
...
plots_buff = plots |> st_transform(3310) |> st_buffer(10+30+70) |> st_simplify(dTolerance = 20)
plot(plots_buff)
...
st_write(st_centroid(plots), file.path(datadir, "for-drone-crew/KML/site-points_ES_v1.kml"), delete_dsn = TRUE)
st_write(plots_buff, file.path(datadir, "for-drone-crew/KML/site-polys_ES_v1.kml"), delete_dsn = TRUE)
...
outdir = file.path(datadir, "for-drone-crew/DSM/ES-all_ellips/ES-all_ellips.tif")
