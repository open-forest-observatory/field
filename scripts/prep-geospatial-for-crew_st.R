# Code to prepare STEF candidate plot locs based on existing datasets
...
# load convex hulls
plots = st_read(file.path(datadir, "field-plot-locs/st-stef/st-stef-focal-polys.gpkg")) |>
  mutate(plot_id = paste0("ST-", str_pad(Name, side = "left", width = 2, pad = "0"))) |>
  dplyr::select(Name = plot_id)

# buffer by the right amount, minimum acceptable drone footprint. 30 m spatial error + 70 m drone photo overlap
plots_buff = plots |> st_transform(3310) |> st_buffer(30+70) |> st_simplify(dTolerance = 20)
plot(plots_buff)
...
st_write(st_centroid(plots), file.path(datadir, "for-drone-crew/KML/site-points_ST_v1.kml"), delete_dsn = TRUE)
st_write(plots_buff, file.path(datadir, "for-drone-crew/KML/site-polys_ST_v1.kml"), delete_dsn = TRUE)
...
outdir = file.path(datadir, "for-drone-crew/DSM/ST-all_ellips/ST-all_ellips.tif")
