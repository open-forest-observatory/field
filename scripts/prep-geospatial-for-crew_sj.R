...
# load convex hulls
plots = st_read(file.path(datadir, "field-plot-locs/sjer/candidate-plot-clusters.gpkg")) |>
  mutate(plot_id = paste0("SJ-", str_pad(priority, side = "left", width = 2, pad = "0"))) |>
...
plots_buff = plots |> st_transform(3310) |> st_buffer(20+30+80) |> st_simplify(dTolerance = 20)
plot(plots_buff)
...
st_write(st_centroid(plots), file.path(datadir, "for-drone-crew/KML/site-points_SJ_v1.kml"), delete_dsn = TRUE)
st_write(plots_buff, file.path(datadir, "for-drone-crew/KML/site-polys_SJ_v1.kml"), delete_dsn = TRUE)
...
outdir = file.path(datadir, "for-drone-crew/DSM/SJ-all_ellips/SJ-all_ellips.tif")
